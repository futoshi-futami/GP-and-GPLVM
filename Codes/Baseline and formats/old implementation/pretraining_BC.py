...
class back_constrained_model(object):
    def __init__(self,D,Q,Hiddenlayerdim1,Hiddenlayerdim2):
        ...
        self.L2_sqr = T.sum(self.hiddenLayer_x.W ** 2)+T.sum(self.hiddenLayer_m.W ** 2)+T.sum(self.hiddenLayer_hidden.W ** 2)+T.sum(self.hiddenLayer_S.W ** 2)
        
        self.error = 0.5*T.sum((self.hiddenLayer_m.output-self.y)**2)/N+0.5*T.sum((self.hiddenLayer_S.output-T.log(0.1))**2)/N
