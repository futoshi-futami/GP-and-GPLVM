...
##################################################################################################
    ...
    def estimate(self,param,i, samples = 20):
        pool = multiprocessing.Pool(samples)    
        #minibatch = np.random.choice(X,batch_size,replace=False)
        arguments = np.ones(samples)
        f_acc=sum(pool.map_async(self.g[param]['LL'], arguments).get(1000))
    ...
    def estimateY(self,param,i, samples = 50):
        ...
        #minibatch = np.random.choice(X,batch_size,replace=False)
        arguments = np.ones(samples)
        f_acc=sum(pool.map_async(self.g[param]['LLY'], arguments).get(1000))
