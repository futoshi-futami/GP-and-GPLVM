...
class DGGPLVM_opt:
    def __init__(self, params, X, Y, samples = 500,batch_size=None):
        self.Y = Y
        self.X = X
        ...
        self.dggplvm = DGGPLVM_model(params,correct,samples=samples,batch_size=None)
        ...
        #RMSPROPのため
        self.param_updates = {n: np.zeros_like(v) for n, v in params.items() if n !=  'X_para'}#update用の同じサイズの空の箱を用意
        self.param_updates['X_para']={n:np.zeros_like(v) for n, v in params['X_para'].items()}
                              
        self.moving_mean_squared = {n: np.zeros_like(v) for n, v in params.items() if n !=  'X_para'}#ＲＭＳＰＲＯＰ用に更新の履歴（γを入れておく器をパラメータごとに用意
        self.moving_mean_squared['X_para'] = {n: np.zeros_like(v) for n, v in params['X_para'].items()}
                                    
        self.learning_rates = {n: 1e-2*np.ones_like(v) for n, v in params.items() if n !=  'X_para'}#行進用の同じサイズの箱を用意
        self.learning_rates['X_para'] = {n: 1e-2*np.ones_like(v) for n, v in params['X_para'].items()}
                              
        #Scipyに入れるよう
        self.opt_param_names = ['Z', 'mu', 'Sigma_b', 'lhyp', 'ls']
        ...
        #2段階用
        self.opt_local_names = ['Wx', 'bx', 'Wu', 'bu', 'Ws', 'bs',"gamma_x","beta_x","gamma_u","beta_u","gamma_s","beta_s"]
        self.opt_local_values = [np.atleast_2d(params['X_para'][n]) for n in self.opt_local_names]
    ...
    def opt_one_step(self, params, iteration, minibatch, opt = 'rmsprop', learning_rate_adapt = 0.2, use_einsum = True):        
        ...
        for param_name in params:
            # DEBUG            
            if param_name in ['X_para']:
                for x_para in self.params['X_para'].keys():
                    self.rmsprop_one_step_minibatch(x_para, minibatch, [x_para, self.X,minibatch], learning_rate_adapt = learning_rate_adapt)#, momentum = 0.9 - 0.4 * 100 / (iteration + 100.0))
    ...
    def rmsprop_one_step_minibatch(self, param_name,minibatch, grad_args, decay = 0.9, momentum = 0, learning_rate_adapt = 0.05, 
        ...
        step1 = self.param_updates['X_para'][param_name] * momentum
        self.params['X_para'][param_name] += step1
        grad = self.get_grad(*grad_args)

        self.moving_mean_squared['X_para'][param_name] = (decay * self.moving_mean_squared['X_para'][param_name] + (1 - decay) * grad ** 2)
        step2 = self.learning_rates['X_para'][param_name]* grad / (self.moving_mean_squared['X_para'][param_name] + 1e-8)**0.5

        self.params['X_para'][param_name] += step2
        ...
        if learning_rate_adapt:
            ...
            step_before_non_negative = self.param_updates['X_para'][param_name]> 0
            agree = (step_non_negative == step_before_non_negative) * 1.#０か１が出る
            adapt = 1 + agree * learning_rate_adapt * 2 - learning_rate_adapt
            self.learning_rates['X_para'][param_name]*= adapt
            self.learning_rates['X_para'][param_name] = np.clip(self.learning_rates['X_para'][param_name], learning_rate_min, learning_rate_max)

        self.param_updates['X_para'][param_name] = step
