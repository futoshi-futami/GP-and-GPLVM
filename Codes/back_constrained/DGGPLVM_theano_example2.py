...
Sigma_b = np.zeros((M,M)) + np.log(0.01)

#zl=np.random.choice(M,batch_size,replace=False)
#Z=m[zl]
Z = np.random.randn(M,Q)

lhyp = np.zeros(Q+1) + np.log(0.1)
ls=np.zeros(1)+np.log(0.1)
        
def init_weights(shape, sample = "xavier"):
    if sample == "uniform":
...
def init_layers(in_size,hidden_size,latent_size):    
    Wx= init_weights((in_size, hidden_size))
    bx = init_bias(hidden_size)
    
    gamma_x=init_bias(hidden_size)+1.
    beta_x=init_bias(hidden_size)
    
    Wu= init_weights((hidden_size, latent_size))
    bu= init_bias(latent_size)
    
    gamma_u=init_bias(latent_size)+1.
    beta_u=init_bias(latent_size)
    
    Ws= init_weights((hidden_size, latent_size))
    bs= init_bias(latent_size)

    gamma_s=init_bias(latent_size)+1.
    beta_s=init_bias(latent_size)
    
    params = {"Wx":Wx, "bx":bx, "Wu":Wu,"bu":bu, "Ws":Ws,"bs":bs,\
              "gamma_x":gamma_x,"beta_x":beta_x,"gamma_u":gamma_u,"beta_u":beta_u,"gamma_s":gamma_s,"beta_s":beta_s}
        
    return params

BC_para=init_layers(D,6,Q)

params = {'X_para':BC_para,'mu':mu,'Sigma_b':Sigma_b,'Z':Z,'lhyp':lhyp,'ls':ls}

#local_params = {'m':m,'S_b':S_b}
#global_params = {'mu':mu,'Sigma_b':Sigma_b,'Z':Z,'lhyp':lhyp,'ls':ls}
    
dggplvm = DGGPLVM_opt(params,X,Y1,batch_size=batch_size)
...
def onestep_opt(batch_size):
    ...
    while iteration < max_iteration:
            ...
            dggplvm.opt_one_step(params.keys(), iteration, minibatch)
            current_ELBO = dggplvm.ELBO(X,N)
            print ('iter ' + str(iteration) + ': ' + str(current_ELBO[0]) + ' +- ' + str(current_ELBO[1]))
            iteration += 1
    elapsed_time = time.time() - start
    print(elapsed_time)
    
def opt_global_local(batch_size):
    iteration = 0
    local_iter = 0
    local_max_iter = 100
    global_iter = 0
    ...
    while iteration < max_iteration:
            ...
            while local_iter < local_max_iter:
                dggplvm.opt_local_step(local_params.keys(), local_iter, minibatch)
                current_ELBO = dggplvm.ELBO(X,N)
                print ('local_iter ' + str(local_iter) + ': ' + str(current_ELBO[0]) + ' +- ' + str(current_ELBO[1]))
                local_iter += 1
            
            while global_iter < global_max_iter:    
                dggplvm.opt_one_step(global_params.keys(), global_iter, minibatch)
                current_ELBO = dggplvm.ELBO(X,N)
                print ('globaliter ' + str(global_iter) + ': ' + str(current_ELBO[0]) + ' +- ' + str(current_ELBO[1]))
                global_iter += 1
