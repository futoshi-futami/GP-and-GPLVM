...
dggplvm = DGGPLVM_opt(N,D,M,Q,Domain_number,X_tr,Weight_share,Xlabel_share,50,Hiddenlayerdim1,Hiddenlayerdim2)
...
def optimization():
    ...
    while iteration < 1000:
        ...
        for index in range(n_train_batches):
            #minibatch = np.random.choice(N,batch_size,replace=False)
            optming_params={}
            dggplvm.opt_one_step_local(iteration,index)
            if index%10==0:
                KL_U=dggplvm.f['KL_U']()
                KL_X=dggplvm.f['KL_X']()
                LL=dggplvm.dggplvm.estimate_f(index,50)
                print ('iter ' + str(iteration) +'_index:' + str(index)+ ': KL_U is:' + str(KL_U))
                print ('KL_X is:' + str(KL_X)+'and LL is ' + str(LL))
        
        index=random.choice(n_train_batches)
        dggplvm.opt_one_step_global(iteration,index)
        KL_U=dggplvm.f['KL_U']()
        KL_X=dggplvm.f['KL_X']()
        LL=dggplvm.dggplvm.estimate_f(index,50)
        print ('iter ' + str(iteration) + ': KL_U is:' + str(KL_U))
