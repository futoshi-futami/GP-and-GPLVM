from DGPLVM_model_cl_s import DGGPLVM_model
import numpy as np
...
class DGGPLVM_opt:
    def __init__(self,D, M,Q,Domain_number,train_set_x,train_label,Y_tr,M_Y,Ydim,batch_size,Hiddenlayerdim1,Hiddenlayerdim2,Y_validate,X_validate,Y_test,X_test,batch_size2,load=None):
        
        
        self.dggplvm = DGGPLVM_model(D, M,Q,Domain_number,M_Y,Ydim,Hiddenlayerdim1,Hiddenlayerdim2)
        
        self.wrt = self.dggplvm.wrt
        
        if load is None:
            self.dggplvm.compile_F(train_set_x,Y_tr,train_label,batch_size)        
        ...
        if load is not None:
            ...
            self.dggplvm.import_F(train_set_x,train_label,batch_size)        
        
        self.global_params=self.dggplvm.global_params
          
        self.dggplvm.prediction_validation(Y_validate,X_validate,Y_test,X_test,batch_size2)
        self.test_model=self.dggplvm.test_model
        self.validate_model=self.dggplvm.validate_model
        ...
        self.moving_mean_squared = {n: np.zeros_like(v.get_value()) for n, v in self.wrt.items()}#ＲＭＳＰＲＯＰ用に更新の履歴（γを入れておく器をパラメータごとに用意
    ...
    def get_grad(self, param_name, index):
        ...
        if param_name in self.global_params:
            ...
            grad2,grad_std=self.estimateY(param_name,index)
            grad = self.dggplvm.g[param_name]['KL_U']() + self.dggplvm.g[param_name]['KL_UY']()+ (grad1+grad2)*self.correct
            
        else:
            grad =  self.dggplvm.g[param_name]['KL_X']() + (self.estimate(param_name,index)[0]+self.estimateY(param_name,index)[0])*self.correct
                                  
        # DEBUG
        if param_name in self.dggplvm.hyp_list and np.any(np.abs(grad) < grad_std / np.sqrt(20)):
                #print 'Large noise, recomputing. lhyp grad mean:', grad, ', std:', grad_std / np.sqrt(self.clgp.samples)
            grad1_ls, grad_std1 = self.estimate(param_name,index,300) 
            grad2_ls,grad_std2=self.estimateY(param_name,index)
            grad = self.dggplvm.g[param_name]['KL_U']() + self.dggplvm.g[param_name]['KL_UY']()+ (grad1_ls+grad2_ls)*self.correct
            self.grad_std = grad_std1+grad_std2

        return np.array(grad)

    def opt_one_step(self, iteration,index, opt = 'rmsprop', learning_rate_adapt = 0.1, use_einsum = True):        
        ...
        for param_name in  self.dggplvm.wrt:
            if opt == 'grad_ascent' or param_name in ['gamma_S','beta_S','b_S','W_S']:#lsはｘの分散
                self.grad_ascent_one_step(param_name, index,[param_name, index], 
                    learning_rate_decay = learning_rate_adapt * 100 / (iteration + 100.0))
            else:
                self.rmsprop_one_step(param_name, index, [param_name, index], learning_rate_adapt = learning_rate_adapt)#, momentum = 0.9 - 0.4 * 100 / (iteration + 100.0))
            if param_name in self.dggplvm.hyp_list:
                self.wrt[param_name].set_value(np.clip(self.wrt[param_name].get_value(), -3, 3))
                
    def opt_one_step_local(self, iteration,index, opt = 'rmsprop', learning_rate_adapt = 0.2, use_einsum = True):        
        for param_name in self.dggplvm.local_params:
            if opt == 'grad_ascent' or param_name in ['S_b']:#lsはｘの分散
                self.grad_ascent_one_step(param_name, index,[param_name, index], 
                    learning_rate_decay = learning_rate_adapt * 100 / (iteration + 100.0))
            else:
                self.rmsprop_one_step(param_name, index, [param_name, index], learning_rate_adapt = learning_rate_adapt)#, momentum = 0.9 - 0.4 * 100 / (iteration + 100.0))
