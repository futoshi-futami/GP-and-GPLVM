...
class Dgrff_model:
    def __init__(self,N_tot,D,Q,Domain_number,Ydim,Hiddenlayerdim1,Hiddenlayerdim2,num_MC,n_rff,free_param):
        ########################################
        ...
        self.RFF_X=RFFLayer(rng, self.Data_input, n_in=D, n_out=Q, num_MC=num_MC,num_FF=n_rff,n_tot=N_tot,free_param=free_param,Domain_number=Domain_number,number="X",Domain_consideration=True)
        ...
        ##############################################################################################
        ###Y側の計算
        self.RFF_Y=RFFLayer(rng, self.RFF_X.output, n_in=Q, n_out=Ydim, num_MC=num_MC,num_FF=n_rff,n_tot=N_tot,free_param=free_param,number="Y",Domain_consideration=False)
    ...
    def cal_check(self,train_set_x,train_set_y,train_label,batch_size):
        ...
        self.train_model_checker=theano.function(
                [index],
                outputs=[self.LL_Y],#self.LL_X + self.LL_Y - self.KL_WX - self.KL_WY - self.KL_hidden + 0.0*sum([T.sum(v) for v in self.params]),
                givens={
                self.X: train_set_x[
                    index * batch_size: (index + 1) * batch_size
                ],
    ...
    def lasagne_optimizer2(self,train_set_x,train_set_y,train_label,batch_size):
        ...
        loss_0 = self.LL_Y  - self.KL_WX - self.KL_WY  + 0.0*sum([T.sum(v) for v in self.params])
        loss=T.cast(loss_0,theano.config.floatX)
