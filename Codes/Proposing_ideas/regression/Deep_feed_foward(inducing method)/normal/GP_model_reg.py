...
class GP_model:
    def __init__(self,N_tot,D_in,D_out,M,Domain_number,dim_int,Hiddenlayerdim1,Hiddenlayerdim2,num_MC,batch_size):
        ########################################
        # set type
        ...
        ##########################################
        ####X側の推論
        self.Gaussian_layer_X=KernelLayer(self.Data_input, D_in=D_in, D_out=dim_int,num_MC=num_MC,inducing_number=M,batch_size=batch_size,Domain_number=None,Domain_consideration=False,number='fix')
    ...
    def lasagne_optimizer(self,train_set_x,train_set_y,train_label,batch_size):
        ...
        loss_0 = self.LL - self.KL_UX - self.KL_UY + 0.0*sum([T.sum(v) for v in self.params])# -0.1*self.MMD
