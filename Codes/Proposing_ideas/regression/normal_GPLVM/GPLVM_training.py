...
#X=X_tr.get_value()
#X_val=X_validate.get_value()
#X_te=X_test.get_value()
#X_te=np.concatenate((X,X_te),0)
...
Domain_number=N_p
Hiddenlayerdim1,Hiddenlayerdim2=500,300
num_MC=10
n_rff=1
df=1
batch_size=500
...
def one_step_opt():
    ...
    while iteration < 50000:
        ...
        for index in range(n_train_batches):
            ...
            alpha=DG.train_model(index)
            print ('iter ' + str(iteration) +'cost is _'+str(alpha) )
            if iteration%10==0:
                LL=DG.f['LL'](index)/N_tot
                #LL_X=DGP.f['LL_X'](index)
                KL_U=DG.f['KL_U'](index)/N_tot
                KL_latent_dim=DG.f['KL_latent_dim'](index)/N_tot
  #              KL_WY=DG.f['KL_WY'](index)
                #MMD=DGP.f['MMD'](index)
                #KL_hidden=DGP.f['KL_hidden'](index)
                
                print(': KL_U is:' + str(KL_U)+ ': LL is:' + str(LL)+ ': KL_latent_dim is:' + str(KL_latent_dim))
            ...
            #    validation_losses = [DGP.validate_model(i)
             #                        for i in range(n_valid_batches)]
              #  this_validation_loss = np.mean(validation_losses,0)
                ...
                #training_losses = [DG.f['LL'](i)-DG.f['KL_U'](i)-DG.f['KL_latent_dim'](i) for i in range(n_train_batches)]
                #test_score=(np.sum(test_losses,0)-np.sum(training_losses,0))/N_te
