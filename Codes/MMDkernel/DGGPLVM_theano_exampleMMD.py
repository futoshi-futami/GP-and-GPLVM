...
Zlabel=np.zeros((M,Domain_number))+np.log(1/Domain_number)

lhyp = np.zeros(Q+1) + np.log(0.1)
ls=np.zeros(Domain_number)+np.log(0.1)
ga=np.zeros(1)+np.log(0.1)

params = {'m':m,'S_b':S_b,'mu':mu,'Sigma_b':Sigma_b,'Z':Z,'lhyp':lhyp,'ls':ls,'ga':ga,'Zlabel':Zlabel}
...
print('start')
dggplvm = DGGPLVM_opt(params,X,Y1,Xinfo,batch_size=batch_size)
