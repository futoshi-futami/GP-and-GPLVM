...
from DGPLVM_model import DGGPLVM_model
from DGPLVM_opt import DGGPLVM_opt
...
#F=dggplvm.compile_F(X_tr,Weight_share,Xlabel_share,50)

dggplvm = DGGPLVM_opt(D,M,Q,Domain_number,X_tr,Weight_share,Xlabel_share,50)
...
def onestep_opt(batch_size):
    ...
    while epoch < max_epoch:
        for index in range(n_train_batches):
            ...
            current_ELBO= dggplvm.f['KL_U']()+dggplvm.f['KL_X']()+dggplvm.dggplvm.estimate_f(index,10)
    ...
    for i in mydict_load[0]:
        dggplvm.wrt[i].set_value(mydict_load[0][i])
        
        
    #これでも保存できる
    np.savez('params.npz', x=save)
    param = np.load('params.npz')
    lis=param['x']
    #lisはオブジェクトなので番号を書かないと呼び出せない
