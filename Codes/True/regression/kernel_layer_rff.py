...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        p_y_given_x, updates = theano.scan(fn=lambda a: T.nnet.softmax(T.dot(a, self.W) + self.b),
                              sequences=[input])

        self.prob=T.mean(p_y_given_x,0)

        # symbolic description of how to compute prediction as class whose
        # probability is maximal
        #self.y_pred = T.argmax(prob, axis=1)
        # end-snippet-1
    ...
    def negative_log_likelihood(self, Xlabel):

        #label=T.argmax(Xlabel, axis=1)
        
        self.LLY = T.mean(T.nnet.categorical_crossentropy(self.prob, Xlabel))
        
        return self.LLY
        
######################################################################        
class KernelLayer(object):
    def __init__(self,input, D_in, D_out,num_MC,inducing_number,Domain_number=None,Domain_consideration=True,number="1",kernel_name='X'):
