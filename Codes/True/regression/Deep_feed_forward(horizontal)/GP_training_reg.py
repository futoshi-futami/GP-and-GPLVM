...
M=50
M_Y=100
fixed_inducings = [X[np.random.choice(N_tot,M)] for i in range(4)]
inducing_points = fixed_inducings

DG = GP_opt(N_tot,D_in,D_out,M,Domain_number,dim_int,Hiddenlayerdim1,Hiddenlayerdim2,num_MC,X_tr,Xlabel_share,Y_tr,batch_size,Y_validate,X_validate,Y_test,X_test,batch_size2,inducing_points)
    ...
    while iteration < 50000:
        ...
        for index in range(n_train_batches):
            ...
            if iteration%10==0:
                ...
                KL_UX=DG.f['KL_UX'](index)
                KL_UY=DG.f['KL_UY'](index)
                #MMD=DGP.f['MMD'](index)
                #KL_hidden=DGP.f['KL_hidden'](index)
                
                print('KL_UX is:' + str(KL_UX)+'KL_UY is:' + str(KL_UY)+ ': LL is:' + str(LL))
