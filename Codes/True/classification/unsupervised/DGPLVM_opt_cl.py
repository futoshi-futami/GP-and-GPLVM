from DGPLVM_model_cl import DGGPLVM_model
import numpy as np
...
class DGGPLVM_opt:
    def __init__(self,D, M,Q,Domain_number,train_set_x,train_label,batch_size,Hiddenlayerdim1,Hiddenlayerdim2,load=None):
        ...
        if load is None:
            self.dggplvm.compile_F(train_set_x,train_label,batch_size)        
        ...
        self.learning_rates = {n: 1e-3*np.ones_like(v.get_value()) for n, v in self.wrt.items()}#行進用の同じサイズの箱を用意                              
    ...
    def get_grad(self, param_name, index):
        ...
        # DEBUG
        if param_name == 'lhyp' and np.any(np.abs(grad) < grad_std / np.sqrt(20)):
