...
class DGGPLVM_model:
    def __init__(self,D, M,Q,Domain_number,Hiddenlayerdim1,Hiddenlayerdim2):
        ...
        ##########################################
        ####X側の推論
        self.Gaussian_layer_X=KernelLayer(rng,self.X,self.hiddenLayer_m.output,self.hiddenLayer_S.output,Q, D,M,Domain_number,self.Xlabel,liklihood='Gaussian',Domain_consideration=True,number='_X')
        ...
        #パラメータの格納
        self.hyp_params={}
        for i in self.Gaussian_layer_X.hyp_params_list:
            self.hyp_params[str(i)]=i
        
        self.Z_params={}
        for i in self.Gaussian_layer_X.Z_params_list:
        ...
        self.LL = self.Gaussian_layer_X.LL
    ...
    def compile_F(self,train_set_x,train_label,batch_size):
        ...
        self.f = {n: theano.function([], f, name=n, givens={self.X: train_set_x,self.Xlabel: train_label
                },on_unused_input='ignore') for n,f in zip(['KL_U', 'KL_X'], [self.KL_U, self.KL_X])}
