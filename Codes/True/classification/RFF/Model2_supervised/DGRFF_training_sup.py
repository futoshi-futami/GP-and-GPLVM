...
from DGRFF_opt_sup import Dgrff_opt
import pickle
import numpy as np
...
Domain_number=N_p
label1=np.zeros((dslr_d['Num'],N_p),dtype=theano.config.floatX)
label1[:,0]=1
label2=np.zeros((web_d['Num'],N_p),dtype=theano.config.floatX)
label2[:,1]=1
label3=np.zeros((cal_d['Num'],N_p),dtype=theano.config.floatX)
label3[:,2]=1
...
def one_step_opt():
    ...
    while iteration < 50000:
        ...
        for index in range(n_train_batches):
            optming_params={}
            #alpha=DGP.dgrff.train_model(index,iteration,0.0001,0)
            ...
            if iteration%50==0:
                ...
                print(elapsed_time)
                #print('LhypX is_:',str(DGP.dgrff.RFF_X.lhyp.get_value()))
                print('LhypX is_:',str(DGP.dgrff.RFF_X.ls.get_value()))
                print('LhypY is_:',str(DGP.dgrff.RFF_Y.lhyp.get_value()))
