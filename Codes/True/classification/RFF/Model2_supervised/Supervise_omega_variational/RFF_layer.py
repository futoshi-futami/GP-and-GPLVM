...
#Build random fourier feature class
class RFFLayer(object):
    def __init__(self, rng, input, n_in, n_out, num_MC,num_FF,Domain_number=None,number="1",Domain_consideration=True):
        ...
        lhyp_values = np.zeros(n_in+1,dtype=theano.config.floatX)+np.log(1.0,dtype=theano.config.floatX)
        ...
        if Domain_consideration:#先行研究は0.1でうまくいった
            ...
            ls_value=np.zeros(1,dtype=theano.config.floatX)+np.log(1.0,dtype=theano.config.floatX)
        ...
        #Define prior omega
        self.prior_mean_omega = T.zeros(n_in)
        self.log_prior_var_Omega=(1/(self.l))**0.5
        
        #Define posterior omega
        mean_omega_value = np.ones((n_in,num_FF))#* 1e-2 
        self.mean_omega = theano.shared(value=mean_omega_value, name='mean_omega'+number, borrow=True)
        
        log_var_omega_value = np.ones((n_in,num_FF))
        self.log_var_omega = theano.shared(value=log_var_omega_value, name='q_omega'+number, borrow=True)
        ...
        Omega_sample=self.sample_Omega_epsilon_0*(T.exp(self.log_var_omega)**0.5)[None,:,:] + self.mean_omega[None,:,:]
        ...
        #KL-divergence
         #Omega
        self.KL_Omega=self.DKL_gaussian(self.mean_omega, self.log_var_omega, self.prior_mean_omega, self.log_prior_var_Omega)
         
         #W
        self.KL_W=self.DKL_gaussian(self.mean_mu, self.log_var_W, prior_mean_W, log_prior_var_W)
        
        #parameter_setting
        self.all_params=[self.lhyp,self.ls,self.mean_mu,self.log_var_W,self.mean_omega,self.log_var_omega]
        self.hyp_params=[self.lhyp,self.ls]
        self.variational_params=[self.mean_mu,self.log_var_W,self.mean_omega,self.log_var_omega]
    ...
    def DKL_gaussian(self,mq, log_vq, mp, log_vp):
        ...
        return 0.5 * T.sum(log_vp_trans - log_vq + ((mq - mp_trans)**2 / T.exp(log_vp_trans))+ T.exp(log_vq - log_vp_trans) - 1)
