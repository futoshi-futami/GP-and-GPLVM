...
class Dgrff_model:
    def __init__(self,N_tot,D,Q,Domain_number,Ydim,Hiddenlayerdim1,Hiddenlayerdim2,num_MC,n_rff):
        ...
        #Q is the latent dimension,D is the data dimension
        ########################
        #hiddenlyaerの作成
        ...
        sample_latent_epsilon = srng.normal((num_MC,N,Q))
        ...
        self.RFF_X=RFFLayer(rng, self.Data_input, n_in=Q, n_out=D, num_MC=num_MC,num_FF=n_rff,Domain_number=Domain_number,number="X",Domain_consideration=True)
        ...
        ##############################################################################################
        ###Y側の計算
        self.RFF_Y=RFFLayer(rng, self.Data_input, n_in=Q, n_out=Ydim, num_MC=num_MC,num_FF=n_rff,number="Y",Domain_consideration=False)
        ...
        #self.KL_OmegaX=self.RFF_X.KL_Omega
        
        self.LL_X =  self.RFF_X.liklihood_nodomain(self.X)*N_tot/(N*num_MC)
        
        #############Y側
        self.LL_Y =  self.RFF_Y.liklihood_nodomain(self.Y)*N_tot/(N*num_MC)
        self.KL_WY = self.RFF_Y.KL_W
        #self.KL_OmegaY=self.RFF_Y.KL_Omega
        ...
        self.KL_latent_dim=self.KLD_X(self.hiddenLayer_m.output,T.exp(self.hiddenLayer_S.output))
    ...
    def lasagne_optimizer(self,train_set_x,train_set_y,train_label,batch_size):
        ...
        loss_0 = self.LL_Y +self.LL_X -self.KL_latent_dim - self.KL_WX - self.KL_WY  + 0.0*sum([T.sum(v) for v in self.params]) # -self.KL_OmegaX  -self.KL_OmegaY#-0.8*self.MMD
    ...
    def cal_check(self,train_set_x,train_set_y,train_label,batch_size):
        ...
        loss = self.LL_Y  - self.KL_WX - self.KL_WY-self.MMD  + 0.0*sum([T.sum(v) for v in self.params])  -self.KL_OmegaX  -self.KL_OmegaY
        ...
        self.train_model_checker=theano.function(
                [index],
                outputs=self.KL_latent_dim,#self.LL_X + self.LL_Y - self.KL_WX - self.KL_WY - self.KL_hidden + 0.0*sum([T.sum(v) for v in self.params]),
                givens={
                self.X: train_set_x[
                    index * batch_size: (index + 1) * batch_size
                ],
    ...
    def lasagne_optimizer2(self,train_set_x,train_set_y,train_label,batch_size):
        ...
        updates2 = lasagne.updates.adam(gparams2, [self.RFF_X.lhyp,self.RFF_X.ls,self.RFF_Y.lhyp], learning_rate = ifelse(T.gt(iteration, 400), a, b))
